...
for i in {renew,backup,restore}; do
  if [ -s $WORK_DIR/$i.sh ]; then
    ...
    # wget -qO- ${GH_PROXY}https://raw.githubusercontent.com/YeeLeer/nezhaserver_V1/main/template/$i.sh | sed '1,/^########/d' >> $TEMP_DIR/$i.sh
    curl -sSL ${GH_PROXY}https://raw.githubusercontent.com/YeeLeer/nezhaserver_V1/main/template/$i.sh | sed '1,/^########/d' >> $TEMP_DIR/$i.sh
