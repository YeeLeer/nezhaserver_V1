...
if [ ! -s /etc/supervisor/conf.d/damon.conf ]; then
  ...
  [ -n "$GH_PROXY" ] && wget --server-response --quiet --output-document=/dev/null --no-check-certificate --tries=2 --timeout=3 https://raw.githubusercontent.com/YeeLeer/docker_nezha_server_V1/refs/heads/main/README.md >/dev/null 2>&1 && unset GH_PROXY
...
EOF
  ...
  wget -qO- ${GH_PROXY}https://raw.githubusercontent.com/YeeLeer/docker_nezha_server_V1/refs/heads/main/template/backup.sh | sed '1,/^########/d' >> $WORK_DIR/backup.sh
...
EOF
    ...
    wget -qO- ${GH_PROXY}https://raw.githubusercontent.com/YeeLeer/docker_nezha_server_V1/refs/heads/main/template/restore.sh | sed '1,/^########/d' >> $WORK_DIR/restore.sh
  fi
...
EOF
  ...
  wget -qO- ${GH_PROXY}https://raw.githubusercontent.com/YeeLeer/docker_nezha_server_V1/refs/heads/main/template/renew.sh | sed '1,/^########/d' >> $WORK_DIR/renew.sh
